...
joined as (
    select
        ...
        coalesce(stripe_cust.customer_email, pg_user.user_email) as customer_email,
        coalesce(stripe_cust.created_at, sf_account.created_at) as created_at
    from stripe_cust
    full outer join sf_account on stripe_cust.customer_name = sf_account.account_name
    full outer join pg_user on stripe_cust.customer_email = pg_user.user_email
)
select * from joined
